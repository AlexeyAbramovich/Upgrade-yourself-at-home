...
import com.abra.workout_at_home.usefulclasses.Timer
...
class FragmentCurrentExercise : Fragment() {
    ...
    private fun getRestTime() {
        viewModel.userInfoLiveData.observe(viewLifecycleOwner, { info ->
            currentUserInfo = info
            if (info.rest_time != 0L) {
                timer.updateRestTime(getCorrectedTimeImMillis(info.rest_time))
            } else {
                timer.updateRestTime(info.rest_time)
            }
        })
